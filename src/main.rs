...
fn make_beeper(enabled: bool, snr_min_db: f32) -> Option<Beeper> {
    if !enabled {
        return None;
    }
    match Beeper::try_new(snr_min_db) {
        Ok(b) => Some(b),
        Err(e) => {
...
fn run_single(cfg: RuntimeConfig, stop: Arc<AtomicBool>) -> Result<()> {
    ...
    let beeper = make_beeper(cfg.beep, cfg.beep_snr_min_db);
    ...
    let mut watcher = SingleSdrWatcher::new(
        ...
        cfg.measure_window_ms,
        cfg.vad_threshold,
        cfg.min_keyed_blocks,
        logger,
        cfg.mode,
        beeper,
    );
